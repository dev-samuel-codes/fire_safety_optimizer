...
### `POST /api/analyze` (multipart: `file`=DWG/DXF, 또는 데모모드)
응답(JSON) — 프론트 `types.ts`와 호환:
```jsonc
{
  "layers":   [{ "id":"fire", "label":"소방", "entityCount":123 }],
  "violations":[{ "id":1, "ruleId":"FV-DET-smoke_12_lt4", "clause":"NFTC 203",
                  "severity":"심각", "description":"강당: 감지기 2개(필요 4개)…",
                  "measured":240, "required":150, "unit":"m2" }],
  "conflicts": [{ "id":1, "severity":"심각", "title":"스프링클러 ↔ 덕트",
                  "location":"강당", "height":"2450mm", "tone":"danger" }],
  "recommendations":[{ "id":1, "title":"대안 1", "summary":"3개 설비 재배치",
                       "saving":"₩8,420,000", "recommended":true }],
  "summary": { "checked":3, "violations":2 }
}
```
프론트는 목업 `conflicts`/`recommendations`를 이 응답으로 **그대로 교체**하면 됨. `violations`는 신규 패널(프론트 `ui-only`의 "NFSC 법규 검토"가 실체화).
